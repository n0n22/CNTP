...
<body>
	...
	<form action="updateBattleResult.bt" method="post" id="submit">
			<input type="hidden" name="battleNo" value="${battleNo}"/>
	        <input type="hidden" id="defeat" name="defeat" value="${battleResult.defeat }"/>
	        <input type="hidden" id="vicRecord" name="vicRecord" value="">
	        <input type="hidden" id="defRecord" name="defRecord" value="">
			 <div class="container h-100">
		        <div class="row h-50 align-items-center" style="margin-top: 50px">
		            <div class="col-12 text-center">
		                <h1 class="font-weight-light">배틀 결과 수정하기</h1>
		                <br>
		            </div>
		        </div>
		        <div class="row">
		                ...
		                <div class="card">
		                    ...
		                    <div class="card-body text-center">
		                        <div style="height: 300px;">
		                            ...
		                            <p class="text-center"></p>
		                            <button type="button" id="homeTeamButton" onclick="select(1)">승리팀</button> <input id="homeTeamRadio" type="radio" name="victory" hidden="hidden" value="${ homeTeam.teamNo }">
		                        </div>
		                    </div>
		                </div>
		            </div>
		                ...
		                <div class="card">
		                    <div class="card-body text-center">
		                        <div style="height: 300px;">
		                            ...
		                            <p class="text-center"></p>
		                            <button type="button" id="awayTeamButton" onclick="select(2)">승리팀</button> <input id="awayTeamRadio" type="radio" name="victory" hidden="hidden" value="${ awayTeam.teamNo }">
		                        </div>
		                    </div>
		                </div>
		            </div>
	        ...
	        <div class="outer-bottom">
	            <div class="submit-area">
	                <br>
	                <button class="btn btn-primary" onclick="submit();">수정하기</button>
	...
	<jsp:include page="../common/footer.jsp"/>
		    ...
		    $(function(){
		    	// 배틀 결과 화면에 뿌려주기
		    	$('#vicRecord').val(JSON.stringify(${battleResult.vicRecord}));
		    	$('#defRecord').val(JSON.stringify(${battleResult.defRecord}));
		    	let vicResult = '';
		    	let defResult = '';
		    	for(var i in ${battleResult.vicRecord}){
		    		let vic = ${battleResult.vicRecord}[i];
		    		let num = Number(i) + 1;
		    		vicResult += 
		    			`<tr class="vics">
		                    <td><input id="vicPlayer\${num}" class="vic" type="text" style="width: 100%" value="\${vic.player}"></td>
		                    <td><input id="vicStyle\${num}" class="vic" type="text" style="width: 100%" value="\${vic.record}"></td>
		                    <td><input id="vicRecord\${num}" class="vic" type="text" style="width: 100%" value="\${vic.style}"></td>
		                </tr>`;
		    	}
		    	for(var i in ${battleResult.defRecord}){
		    		let def = ${battleResult.defRecord}[i];
		    		let num = Number(i) + 1;
		    		defResult +=
	    			    `<tr class="defs">
		                    <td><input id="defPlayer\${num}" class="def" type="text" style="width: 100%" value="\${def.player}"></td>
		                    <td><input id="defStyle\${num}" class="def" type="text" style="width: 100%" value="\${def.record}"></td>
		                    <td><input id="defRecord\${num}" class="def" type="text" style="width: 100%" value="\${def.style}"></td>
		                </tr>`;		    			
		    			
		    	}
		    	$('#victoryTeam tbody').html(vicResult);
		    	$('#defeatTeam tbody').html(defResult);
		    	$('input[name="victory"]').each(function(){
		    		var $this = $(this);
		    		if($this.val() == '${battleResult.victory}'){
		    			$this.attr("checked", true);
		    			$this.prev().attr("class", "btn btn-lg btn-primary");
		    		} else{
		    			$this.prev().attr("class", "btn btn-lg btn-light");
		    		}
		    	})
